...
from lst_utils import read_list
...
def image_encode_old(args, i, item, q_out):
...
def image_encode_pass_through(args, i, item, q_out):

    fullpath = os.path.join(args.root, item[1])
    header = get_header(args, item)
...
def image_encode(args, i, item, q_out, img=None):
...
def read_worker(args, q_in, q_out):
    while True:
        ...
        if args.pass_through:
            image_encode_pass_through(args, i, item, q_out)
        else:
            ...
            if fname.startswith(args.prefix) and fname.endswith('.lst'):
                ...
                if args.num_thread > 1 and multiprocessing is not None:
                    ...
                    try:
                        ...
                        import queue
                    q_out = queue.Queue()
                    augmented_images_q_out = queue.Queue()
                    fname = os.path.basename(fname)
                    fname_rec = os.path.splitext(fname)[0] + '.rec'
                    fname_idx = os.path.splitext(fname)[0] + '.idx'
                    record = mx.recordio.MXIndexedRecordIO(os.path.join(working_dir, fname_idx),
                    ...
                    for i, item in enumerate(image_list):
                        ...
                        if args.pass_through:
                            image_encode_pass_through(args, image_idx, item, q_out)
                            image_idx += 1
                        else:
                            # read image in advance
                            fullpath = os.path.join(args.root, item[1])
                            img = cv2.imread(fullpath, args.color)
                    ...
                    while not augmented_images_q_out.empty():
                        this_idx, s, _ = augmented_images_q_out.get()

                        record.write_idx(tot_img+this_idx, s)
